define(['dispatcher', 'stage-color/stage-color.store'], function(dispatcher, store) {
	...
	var _handleChange = function(storeData) {
		...
		for (var i = 0; i < lines.length; i++) {
			for (var j = 0; j < lines[i].rows.length; j++) {
				rand = Math.floor(Math.random()*Math.random()*2)*timeFactor + 600 + 'ms';
				lines[i].rows[j].element.style[transitionDelay] = rand;
			}
		}
		...
		var buldNet = function() {
			...
			var setDelays = function(line, i) {
				...
				for (var j = 0; j < line.rows.length; j++) {
					...
					// if (i < lp1 || i >= line.rows.length - lp1 || j < rp1 || j >= line.rows.length - rp1) {
					// 	line.rows[j].element.style[transitionDelay] = (i*timeFactor + rowsFactor[j]) + 'ms';
					// } else {
					// 	line.rows[j].element.style[transitionDelay] = ((numLines*timeFactor - i*timeFactor) + rowsFactor[j]) + 'ms';
					// }

					line.rows[j].element.style[transitionDelay] = rand;
				}
			}
